...
export async function orderAction(
    ...
    let order: IOrder = {
        ...
        cashSum: (formData.get('cashSum') as unknown as number) || 0,
        paymentStatus: false,
...
export async function updateUserInfo(prevState: { message: string }, formData: FormData) {
    const updatedUser = {
    ...
    if (!res.ok) {
        ...
        return {
            message: `Error! Failed to change user data: ${str}`,
        };
    }
